...
module.exports.getCoordinate = function (S, E) {
  return [
    module.exports.scaleSystemizing(S),
    module.exports.scaleEmpathizing(E)
  ];
}
...
function test(E, S) {
  ...
  var c = module.exports.getCoordinate(S, E);
  console.log('EQ:%s\tSQ:%s\tD:%s\t%s  %sx %sy', 
    E, 
    S, 
    eqNum(getDeviation(S, E).toFixed(2)), 
    b.paddingLeft('            '),
    c[0].toFixed(1).paddingLeft('      '),
    c[1].toFixed(1).paddingLeft('      '));  
}
