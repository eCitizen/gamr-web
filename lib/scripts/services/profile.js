...
var normalize = {
  personality: function(key, value) {
    return value * 2;
  },
  brainType: function(key, value) {
    if (key === 'Empathizing') {
      return (value - 23.8) / 8.75; // z(EQ) = (EQ - 23.8) / 8.75
    } else if (key === 'Systemizing') {
      return (value - 19) / 10.05; // z(SQ) = (SQ - 19) / 10.05
    } else {
  ...
  for (surveyName in unpacked) {
    if (surveyKey[surveyName]) {
      ...
      surveyKey[surveyName].forEach(function (dimension, idx) {
        ...
        if (normalize[surveyName]) {
          value = normalize[surveyName](dimension.title, value);
        }

        friendly[surveyName][dimension.title] = value;

        if (surveyName === 'gamerType') friendly.gamerTypeKey += value;
