...
function packProfile(profile, k) {
  var f = [],
    _ = mP(f);

  pO.forEach(function (n) {
    if (k[n].length !== profile[n].length) {
      throw new Error('key/profile mismatch: ' + n);
    };

    k[n].forEach(function (a, idx) {
      var v = profile[n][idx], c = a.total ? (a.total - v) : (a.options.indexOf(v) === 0 ? a.options[1] : a.options[0]);
      _(hV(v));
      _(hV(c));
    });
  });
...
function unpackProfile(p, k) {
  try {
    ...
    pO.reverse().forEach(function (n) {
      f[n] = [];
      k[n].reverse().forEach(function (a) {
        var c = eV(_()), v = eV(_());
...
var p = packProfile(profile, surveyKey);

console.log(p)

console.log(packProfile(unpackProfile(p, surveyKey)));
